...
#include<directX/Drawable/Geometric primitives/Cone.h>
	...
	phi(adist(rng))
{
	...
	if (!IsStaticInitialized())
	{
		...
		auto pvs = std::make_unique<VertexShader>(gfx, "ColorBlendVS.cso");
		auto pvsbc = pvs->GetBytecode();
		AddStaticBind(std::move(pvs));

		AddStaticBind(std::make_unique<PixelShader>(gfx, "ColorBlendPS.cso"));
